...
class RestExceptionHandler() : ResponseEntityExceptionHandler() {
    ...
    private val logger =
        LoggerFactory.getLogger(delta.timewarp.server.exception.RestExceptionHandler::class.java)
    ...
    protected fun handleCustomException(
        error: delta.timewarp.server.exception.CustomAuthException
    ): ResponseEntity<Any> {
        return ResponseEntity.status(error.status)
            .body(delta.timewarp.server.dtos.ErrorMessageDTO(error.message.toString()))
    }

    @ExceptionHandler(delta.timewarp.server.exception.CustomException::class)
    protected fun handleCustomException(
        error: delta.timewarp.server.exception.CustomException
    ): ResponseEntity<Any> {
        return ResponseEntity.status(error.status)
            .body(delta.timewarp.server.dtos.ErrorMessageDTO(error.message.toString()))
    }
    ...
    protected fun handleRuntimeException(
        error: Exception
    ): ResponseEntity<delta.timewarp.server.dtos.ErrorMessageDTO> {
